...
| 方式 | 适合场景 | 优点 |
| --- | --- | --- |
| 极简本地 | 只想在本机终端检查 Surge | 最轻量；不需要 Hermes 或 Codex；脚本直接调用 `surge-cli` |
| Hermes 推荐 | 希望常驻巡检、异常通知和持续学习 | 健康时完全静默；重要问题再唤醒 AI；可通过聊天工具推送 |
| Codex 可选 | 希望低频检查仓库、复盘异常、维护项目 | 适合每日/每周自动化；不建议做每分钟巡检 |
