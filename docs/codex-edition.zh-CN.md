...
```text
请把 https://github.com/rexchen2024/surge-guardian-assistant 安装到本地，作为 Surge 守护助手项目使用。请运行 doctor 和 scripts/check，然后根据 codex/automation-prompts/surge-guardian-review.md 创建或建议一个安全的 Codex 自动化。不要在未确认前编辑 Surge profile、证书、DNS、服务器、MITM、Rewrite、Scripting、Replica，也不要执行 reload 或 restart。
```
