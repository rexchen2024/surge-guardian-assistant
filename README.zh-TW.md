...
- [Surge](https://nssurge.com/)：macOS / iOS 網路與代理工具。本專案負責檢查和守護 Surge。
- [Hermes](https://github.com/NousResearch/hermes-agent)：定時執行、異常分析和訊息通知層。建議用於常駐巡檢。
- [Codex](https://openai.com/codex/)：OpenAI 的程式碼助手。適合低頻檢查、異常回顧和專案維護。
...
守護助手只做低風險、可回復的處理，例如重試外部資源、刷新 DNS、複測策略、加入執行階段臨時規則。永久修改 Surge profile、重啟服務、憑證、DNS、MITM、Rewrite、Scripting 等操作都需要使用者確認。

## 專案邊界

本專案不是 Surge 設定庫、規則集、模組合集或機場推薦。它只負責在你已有 Surge 設定的基礎上做巡檢、自我修復和異常回饋。
...
專案不會自動上傳日誌或使用資料。使用者可以主動產生去識別化報告，檢查後再決定是否提交：
